// ============================================================
    ...
    function warmUpPostImages(postList) {
      ...
      limited.forEach(function(post, postIndex) {
        const images = post && post.images && Array.isArray(post.images) && post.images.length > 0
          ? post.images
          : (post && post.imageUrl ? [post.imageUrl] : []);
        images.slice(0, 2).forEach(function(url, imageIndex) {
          if (!url) return;
          queueImagePrefetch(url, postIndex === 0 && imageIndex === 0 ? 'high' : 'low');
        });
      });
    }
    ...
    function loadPostCardMediaSequentially(card) {
      ...
      images.forEach(function(img, index) {
        chain = chain.then(function() {
          if (!isHomeScreenActive() && index > 0) {
            return Promise.resolve();
          }
          return loadLazyMediaImage(img).then(function() {
            return new Promise(function(resolve) {
              setTimeout(resolve, index === 0 ? 12 : 36);
            });
          });
        });
      });
    ...
    function createCarouselSlide(url, isPrimary) {
      ...
      try { img.loading = isPrimary ? 'eager' : 'lazy'; } catch (err) {}
    ...
    function showDownloadDialog(postId) {
      ...
      images.forEach((url, index) => {
        ...
        try { thumbImg.loading = 'lazy'; } catch (err) {}
        if ('fetchPriority' in thumbImg) {
          try { thumbImg.fetchPriority = 'low'; } catch (err) {}
        }
        thumbImg.src = url;
